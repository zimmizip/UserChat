...
function zzimPro(index, str, dname) {								
    ...
    location.href="goodProc.jsp?dname="+encodeURIComponent(dname)+"&button="+encodeURIComponent("찜하기");		
...
<% 
Vector<datecBean> dateclist = new Vector<datecBean>();
Vector<datecBean> datekindlist = new Vector<datecBean>();
String location = (String)session.getAttribute("dlocation");
Vector<datecBean> datesearchlist = new Vector<datecBean>();								/* datesearchlist => 검색 후 저장할 list */

String kind=(String)session.getAttribute("kind"); 
String category = request.getParameter("category");
String sentence = request.getParameter("sentence");

if(category!=null&&sentence!=null){														/* category, sentence 값이 null이 안된상태 */
	datesearchlist = mMgr.getkindList(null,null,category,sentence);						/* 검색결과를 datesearchlist에 저장 */
}
%>
...
<% 
   for(int i=0; i<datesearchlist.size(); i++){													/* datesearchlist 뿌려주는 코드 */
%>
<table border = "1px solid black" width="1000" height="300" bgcolor="white" >

<tr>
<th width="300" rowspan="6" bgcolor="white" >
   <font size="2" color="#666666">
   <img  width = "300" height = "300" src = "/UserChat/ny/<%=datesearchlist.get(i).getDphoto() %>">
  
     <%  boolean mgrchk = mMgr.goodcheck("test",datesearchlist.get(i).getDname()); %>
  <% if(mgrchk){ %>
   <input type="button" class="total" value= "찜취소하기" name="good"
   onclick = "zzimPro('<%=i %>', 'total', '<%=datesearchlist.get(i).getDname() %>')" />
   <%}else{ %>
   ...
   <td><b><%=datesearchlist.get(i).getDname() %></b></td></tr>
   <tr><td><%=datesearchlist.get(i).getDexplan() %></td></tr>
   <tr><td><b>TEL :</b><%=datesearchlist.get(i).getDtel() %></td></tr>
   <tr><td><b>메뉴 및 가격 :</b><%=datesearchlist.get(i).getDmenu() %> - <%=datesearchlist.get(i).getDprice()%></td></tr>
   <tr><td><b>주소 :</b> <%=datesearchlist.get(i).getDaddress() %>&nbsp;&nbsp;&nbsp;
   <input type="button" class = "btn btn-primary" name="map" value="지도보기"	
   onclick="window.open('/UserChat/ny/maps_ny.jsp?address=<%=datesearchlist.get(i).getDaddress()%>&name=<%=datesearchlist.get(i).getDname()%>','window팝업','width=400, height=200, menubar=no, status=no, toolbar=no');" />
   </td></tr>
   <tr><td><b>영업시간 :</b><%=datesearchlist.get(i).getDtime() %></td></tr>
   </td>
</table>
<% 
   }
...
function openCity(type) {									/* type 메뉴 선택시 type에 해당하는 테이블 출력하는 함수	/  cityName => kind(type) */
	  ...
	  location.href="place.jsp?name="+type;
	}
</script>
</body>
</html>